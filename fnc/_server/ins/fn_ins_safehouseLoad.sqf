...
_centerObj = createVehicle["Land_Bollard_01_F", [0,0,0], [], 0, "CAN_COLLIDE"];
_centerObj allowDamage false;

_obj1 = ["CUP_USSpecialWeapons_EP1", _centerObj, _dataArrays select 1, _dataArrays select 2] call _fnc_CreateVehicle;
_obj2 = ["Box_CSAT_Equip_F", _centerObj, _dataArrays select 3, _dataArrays select 4] call _fnc_CreateVehicle;
_obj3 = ["Box_CSAT_Uniforms_F", _centerObj, _dataArrays select 5, _dataArrays select 6] call _fnc_CreateVehicle;

_safehouseData = [_position, _obj1, _obj2, _obj3];
_safehouseDataArray = missionNamespace getVariable["server_safehouseData", []];
