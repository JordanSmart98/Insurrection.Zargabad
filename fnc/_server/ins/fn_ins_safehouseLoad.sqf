...
private _fnc_CreateVehicle = {
    params["_safehouse", "_type", "_centerObj", "_attachToOffset", "_vectorDirAndUp"];
    ...
    _obj setVariable ["local_safehouse", _safehouse];
    _obj addEventHandler ["Killed", {call server_fnc_ins_safehouseEH;}];
    _obj;
};
...
private _dataArrays = [];
{
    ...
    if (_name == (typeOf _building)) then {_dataArrays = _x;};
...
private _obj1 = [_building, "CUP_USSpecialWeapons_EP1", _centerObj, _dataArrays select 1, _dataArrays select 2] call _fnc_CreateVehicle;
private _obj2 = [_building, "Box_CSAT_Equip_F", _centerObj, _dataArrays select 3, _dataArrays select 4] call _fnc_CreateVehicle;
